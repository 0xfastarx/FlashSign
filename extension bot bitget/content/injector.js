...
window.addEventListener('message', async (event) => {
  ...
  // ── RPC Request dari ethereum-provider.js (Aptos) ────────────────────────
  if (event.data.channel === APTOS_FASTARX_CHANNEL) {
    const { id, method, params } = event.data;

    try {
      const response = await chrome.runtime.sendMessage({
        action: 'aptosRpcRequest',
        id,
        method,
        params,
        origin: window.location.origin
      ...
      if (!response) {
        window.postMessage({
          channel: APTOS_FASTARX_CHANNEL + '_response',
          id,
          error: { code: -32603, message: 'Aptos: No response' }
        }, '*');
        return;
      }
  ...
  // ── RPC Request dari ethereum-provider.js (TON) ──────────────────────────
  if (event.data.channel === TON_FASTARX_CHANNEL) {
    const { id, method, params } = event.data;

    try {
      const response = await chrome.runtime.sendMessage({
        action: 'tonRpcRequest',
        id,
        method,
        params,
        origin: window.location.origin
      ...
      if (!response) {
        window.postMessage({
          channel: TON_FASTARX_CHANNEL + '_response',
          id,
          error: { code: -32603, message: 'TON: No response' }
        }, '*');
        return;
      }
  ...
  // ── DApp Disconnect Event (Aptos) ─────────────────────────────────────────
  if (event.data.channel === APTOS_FASTARX_CHANNEL + '_dapp_disconnect') {
    const { origin, reason } = event.data;
    console.log('[Aptos] 📤 Forwarding disconnect:', origin, reason);

    try {
      await chrome.runtime.sendMessage({
        action: 'aptosDappDisconnect',
        origin: origin || window.location.origin,
        reason: reason || 'unknown'
      });
    } catch (err) {
      console.warn('[Aptos] Disconnect relay failed:', err.message);
    }
    return;
  }

  // ── DApp Disconnect Event (TON) ───────────────────────────────────────────
  if (event.data.channel === TON_FASTARX_CHANNEL + '_dapp_disconnect') {
    const { origin, reason } = event.data;
    console.log('[TON] 📤 Forwarding disconnect:', origin, reason);

    try {
      await chrome.runtime.sendMessage({
        action: 'tonDappDisconnect',
        origin: origin || window.location.origin,
        reason: reason || 'unknown'
      });
    } catch (err) {
      console.warn('[TON] Disconnect relay failed:', err.message);
    }
    return;
  }
});
...
chrome.runtime.onMessage.addListener((message) => {
  ...
  if (message.type === 'APTOS_PROVIDER_EVENT') {
    window.postMessage({
      channel: APTOS_FASTARX_CHANNEL + '_event',
      event: message.event,
      data: message.data
    }, '*');
  }
  if (message.type === 'TON_PROVIDER_EVENT') {
    window.postMessage({
      channel: TON_FASTARX_CHANNEL + '_event',
      event: message.event,
      data: message.data
    }, '*');
  }
